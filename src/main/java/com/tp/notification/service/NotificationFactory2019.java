...
class NotificationFactory2019 {
    ...
    private List<Файл.Документ.УвКонтрСд.СвКонтрСд> transactionsFilling(){
        List<Файл.Документ.УвКонтрСд.СвКонтрСд> свКонтрСдСписок = null;
        if(transactions.size() != 0){
            свКонтрСдСписок = new ArrayList<>();
            for(String[] item : transactions){
                Файл.Документ.УвКонтрСд.СвКонтрСд свКонтрСд = new Файл.Документ.УвКонтрСд.СвКонтрСд();
    ...
    private List<Файл.Документ.УвКонтрСд.СвКонтрСд.СвОргУчаст> свОргУчастМетод(Integer numOfTransaction){
        List<Файл.Документ.УвКонтрСд.СвКонтрСд.СвОргУчаст> свОргУчастСписок = null;
        if(partiesOrganizations.size() != 0){
            ...
            for(String[] item : partiesOrganizations){
                if(Integer.parseInt(item[0]) == numOfTransaction){
                    Файл.Документ.УвКонтрСд.СвКонтрСд.СвОргУчаст свОргУчаст = new Файл.Документ.УвКонтрСд.СвКонтрСд.СвОргУчаст();
    ...
    private Файл.Документ.УвКонтрСд.СвКонтрСд.СвФЛУчаст.СвФЛ свФЛМетод(int index){
        Файл.Документ.УвКонтрСд.СвКонтрСд.СвФЛУчаст.СвФЛ свФЛ = null;
        ...
        if(датаРожд != null || местоРожд != null || гражд != null || оксм != null ||
            ...
            свФЛ = new Файл.Документ.УвКонтрСд.СвКонтрСд.СвФЛУчаст.СвФЛ();
    ...
    private List<Файл.Документ.УвКонтрСд.СвКонтрСд.СвФЛУчаст> свФЛУчастМетод(Integer numOfTransaction){
        List<Файл.Документ.УвКонтрСд.СвКонтрСд.СвФЛУчаст> свФЛУчастСписок = null;
        if(partiesPersons.size() != 0){
            свФЛУчастСписок = new ArrayList<>();
            int counter = 0;
            for(String[] item : partiesPersons){
                if(Integer.parseInt(item[0]) == numOfTransaction){
                    Файл.Документ.УвКонтрСд.СвКонтрСд.СвФЛУчаст свФЛУчаст = new Файл.Документ.УвКонтрСд.СвКонтрСд.СвФЛУчаст();
    ...
    private Файл.Документ.УвКонтрСд.СвКонтрСд.СвПредмСдН.ПерПредСд перПредСдМетод(int index){
        Файл.Документ.УвКонтрСд.СвКонтрСд.СвПредмСдН.ПерПредСд перПредСд = null;
        ...
        if(наимПредСд != null || тнвэд != null || окпд2 != null || оквэд2 != null
            ...
            перПредСд = new Файл.Документ.УвКонтрСд.СвКонтрСд.СвПредмСдН.ПерПредСд();
    ...
    private List<Файл.Документ.УвКонтрСд.СвКонтрСд.СвПредмСдН> свПредмСдНМетод(Integer numOfTransaction){
        List<Файл.Документ.УвКонтрСд.СвКонтрСд.СвПредмСдН> свПредмСдНСписок = null;
        if(subjects.size() != 0){
            свПредмСдНСписок = new ArrayList<>();
            Map<String, Файл.Документ.УвКонтрСд.СвКонтрСд.СвПредмСдН> map = new HashMap<>();
            int counter = 0;
            for(String[] item : subjects){
                if(Integer.parseInt(item[0]) == numOfTransaction){
                    if(!map.containsKey(item[1])){
                        Файл.Документ.УвКонтрСд.СвКонтрСд.СвПредмСдН свПредмСдН = new Файл.Документ.УвКонтрСд.СвКонтрСд.СвПредмСдН();
                        свПредмСдН.setТипПредСд(item[1]);
                        List<Файл.Документ.УвКонтрСд.СвКонтрСд.СвПредмСдН.ПерПредСд> перПредСдСписок = new ArrayList<>();
                        перПредСдСписок.add(перПредСдМетод(counter));
                        свПредмСдН.setПерПредСд(перПредСдСписок);
                        map.put(item[1], свПредмСдН);
                    }
                    else{
                        Файл.Документ.УвКонтрСд.СвКонтрСд.СвПредмСдН свПредмСдН = map.get(item[1]);
                        свПредмСдН.getПерПредСд().add(перПредСдМетод(counter));
                    }
                }
                counter++;
            }
            for(Map.Entry<String, Файл.Документ.УвКонтрСд.СвКонтрСд.СвПредмСдН> item : map.entrySet()){
    ...
    private Файл.Документ.УвКонтрСд.СвКонтрСд.ДохРасхСд дохРасхСдМетод(Integer numOfTransaction){
        Файл.Документ.УвКонтрСд.СвКонтрСд.ДохРасхСд дохРасхСд = null;
        String сумДохСд = transactions.get(numOfTransaction)[35];
        String сумДохСдРег = transactions.get(numOfTransaction)[36];
        String сумРасхСд = transactions.get(numOfTransaction)[37];
        String сумРасхСдРег = transactions.get(numOfTransaction)[38];
        if(сумДохСд != null || сумДохСдРег != null || сумРасхСд != null || сумРасхСдРег != null){
            дохРасхСд = new Файл.Документ.УвКонтрСд.СвКонтрСд.ДохРасхСд();
    ...
    private Файл.Документ.УвКонтрСд.СвКонтрСд.КонтрСд.КодИстИнф кодИстИнфМетод(Integer numOfTransaction){
        Файл.Документ.УвКонтрСд.СвКонтрСд.КонтрСд.КодИстИнф кодИстИнф = null;
        String ист251 = transactions.get(numOfTransaction)[26];
        String ист252 = transactions.get(numOfTransaction)[27];
        String ист253 = transactions.get(numOfTransaction)[28];
        String ист254 = transactions.get(numOfTransaction)[29];
        String ист255 = transactions.get(numOfTransaction)[30];
        String ист256 = transactions.get(numOfTransaction)[31];
        String ист257 = transactions.get(numOfTransaction)[32];
        String ист258 = transactions.get(numOfTransaction)[33];
        String ист259 = transactions.get(numOfTransaction)[34];
        if(ист251 != null || ист252 != null || ист253 != null || ист254 != null || ист255 != null ||
                ист256 != null || ист257 != null || ист258 != null || ист259 != null){
            кодИстИнф = new Файл.Документ.УвКонтрСд.СвКонтрСд.КонтрСд.КодИстИнф();
    ...
    private Файл.Документ.УвКонтрСд.СвКонтрСд.КонтрСд контрСдМетод(Integer numOfTransaction){
        Файл.Документ.УвКонтрСд.СвКонтрСд.КонтрСд контрСд = null;
        Файл.Документ.УвКонтрСд.СвКонтрСд.КонтрСд.КодИстИнф кодИстИнф = кодИстИнфМетод(numOfTransaction);
        String групОС = transactions.get(numOfTransaction)[15];
        String кодНаимСд = transactions.get(numOfTransaction)[16];
        String кодСторСд = transactions.get(numOfTransaction)[17];
        String прОпрЦен = transactions.get(numOfTransaction)[18];
        String комПрОпрЦен = transactions.get(numOfTransaction)[19];
        String кодОпрЦен = transactions.get(numOfTransaction)[20];
        String комКодОпрЦен = transactions.get(numOfTransaction)[21];
        String кодМетЦен = transactions.get(numOfTransaction)[22];
        String комКодМетЦен = transactions.get(numOfTransaction)[23];
        String колУчСд = transactions.get(numOfTransaction)[24];
        String комКолУчСд = transactions.get(numOfTransaction)[25];
        if(кодИстИнф != null || групОС != null || кодНаимСд != null || кодСторСд != null || прОпрЦен != null ||
            ...
            контрСд = new Файл.Документ.УвКонтрСд.СвКонтрСд.КонтрСд();
    ...
    private Файл.Документ.УвКонтрСд.СвКонтрСд.ОснКонтрСд.Осн10514 осн10514Метод(Integer numOfTransaction){
        Файл.Документ.УвКонтрСд.СвКонтрСд.ОснКонтрСд.Осн10514 осн10514 = null;
        ...
        if(осн121 != null || осн122 != null || осн123 != null || осн124 != null){
            осн10514 = new Файл.Документ.УвКонтрСд.СвКонтрСд.ОснКонтрСд.Осн10514();
    ...
    private Файл.Документ.УвКонтрСд.СвКонтрСд.ОснКонтрСд.ОснРФ10514 оснРФ10514Метод(Integer numOfTransaction){
        Файл.Документ.УвКонтрСд.СвКонтрСд.ОснКонтрСд.ОснРФ10514 оснРФ10514 = null;
        ...
        if(осн131 != null || осн132 != null || осн133 != null || осн134 != null
            ...
            оснРФ10514 = new Файл.Документ.УвКонтрСд.СвКонтрСд.ОснКонтрСд.ОснРФ10514();
    ...
    private Файл.Документ.УвКонтрСд.СвКонтрСд.ОснКонтрСд оснКонтрСдМетод(Integer numOfTransaction){
        Файл.Документ.УвКонтрСд.СвКонтрСд.ОснКонтрСд оснКонтрСд = null;
        Файл.Документ.УвКонтрСд.СвКонтрСд.ОснКонтрСд.Осн10514 осн10514 = осн10514Метод(numOfTransaction);
        Файл.Документ.УвКонтрСд.СвКонтрСд.ОснКонтрСд.ОснРФ10514 оснРФ10514 = оснРФ10514Метод(numOfTransaction);
        String взЗавис = transactions.get(numOfTransaction)[2];
        if(осн10514 != null || оснРФ10514 != null || взЗавис != null){
            оснКонтрСд = new Файл.Документ.УвКонтрСд.СвКонтрСд.ОснКонтрСд();
    ...
    Файл populateClasses(){
        ...
        Файл.Документ.СвНП.НПФЛ.СвФЛ свФЛ = null;
        if(удЛичнФЛТип != null || адрИнТип != null || taxpayer.get(43) != null
            ...
            свФЛ = new Файл.Документ.СвНП.НПФЛ.СвФЛ();
            свФЛ.setУдЛичнФЛ(удЛичнФЛТип);
            свФЛ.setАдрИн(адрИнТип);
        ...
        Файл.Документ.Подписант.СвПред свПред = null;
        if (taxpayer.get(26) != null || taxpayer.get(27) != null){
            свПред = new Файл.Документ.Подписант.СвПред();
        ...
        Файл.Документ.Подписант подписант = null;
        if (фиоТип1 != null || свПред != null || taxpayer.get(20) != null){
            подписант = new Файл.Документ.Подписант();
            подписант.setФИО(фиоТип1);
            подписант.setСвПред(свПред);
            подписант.setПрПодп(taxpayer.get(20));
        }

        List<Файл.Документ.УвКонтрСд.СвКонтрСд> свКонтрСд = transactionsFilling();
        Файл.Документ.УвКонтрСд увКонтрСд = null;
        if(свКонтрСд != null){
            увКонтрСд = new Файл.Документ.УвКонтрСд();
            увКонтрСд.setСвКонтрСд(свКонтрСд);
        }

        Файл.Документ документ = null;
        ...
        if (документ != null || taxpayer.get(1) != null || taxpayer.get(2) != null
            ...
            файл = new Файл();
            файл.setДокумент(документ);
            файл.setИдФайл(taxpayer.get(1));
            файл.setВерсПрог(taxpayer.get(2));
            файл.setВерсФорм(taxpayer.get(3));
            файл.setИдФайлИсх(taxpayer.get(4));
            файл.setИдФайлПерв(taxpayer.get(5));
            файл.setКолФайл(taxpayer.get(6));
            файл.setНомФайл(taxpayer.get(7));
        }
        return файл;
    }
}
